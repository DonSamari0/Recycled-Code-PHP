<?php
    require 'conn.php';
    require 'cabecera.php'
?>

<h4>Opciones del sistema de informacion</h4>
    <div class="row">
    
        <div class="col-sm">
            <div class="form-group">
                
                <a name="" id="" class="btn btn-primary" href="crear_lugar.php" role="button">Crear Lugar</a>
                <a name="" id="" class="btn btn-primary" href="crear_especie.php" role="button">Crear Especie</a>
                <a name="" id="" class="btn btn-primary" href="consultar.php" role="button">Consultar</a>
                
            </div>   
        </div>
        <div class="col-sm">
        <div class="form-group">
                
                <div class="form-group">
                  <input type="text" class="form-control" name="" id="" aria-describedby="helpId" placeholder="Digite el nombre o alias de la especie a consultar">
                  <a name="" id="" class="btn btn-primary" href="busqueda.php" role="Buscar">Buscar</a>  
                </div>
                
            </div>
        </div> 
    </div>
</div>
    
</body>
</html>